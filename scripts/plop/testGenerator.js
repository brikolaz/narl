...
const TEST_TEMPLATE_PATH = fileURLToPath(
  new URL("./templates/test/test.ts.hbs", import.meta.url),
)
...
export const resolveDestination = async ({
  ...
  if (matches.length > 1) {
    if (!chooseMatch) {
      throw new Error(
        "Multiple source matches require an interactive selection",
      )
    }
...
export const renderTest = async ({
  ...
  const replacements = {
    describeName,
    gameImport: toImportPath(
      destinationDirectory,
      path.join(repoRoot, "src/game"),
    ),
    internalActionTypeImport: toImportPath(
      destinationDirectory,
      path.join(repoRoot, "src/game/systems/internal/types"),
    ),
