...
const createExplosiveEntity = (state: GameState, position = 10): Entity => {
  ...
  upsertRoleEntities(explosive, {
    [EntityRoleEnum.BACKPACK]: ContainerEntityFactory.getBackpack(),
  })
...
describe("Explode action cycle", () => {
  ...
  it("runs the complete immediate cycle through game dispatch", () => {
    ...
    expect(
      game.state.log.some(
        ({ action }) => action.type === WorldActionTypeEnum.WORLD_ATTACK,
      ),
    ).toBe(false)
  })

  it.each([ExplodeComponent, ExplodeRangeComponent])(
    "does not start the cycle when %s is missing",
    (missing) => {
      const explosive = createExplosiveEntity(game.state)
