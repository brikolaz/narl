...
const getPursuers = (): Entity[] => getAllMobs().filter(isPursuer)
...
const getValidSpawnPositions = (playerPosition: number): number[] => {
  ...
  return STATE.world
    ...
    .filter(
      (tile) =>
        canSpawnMobAt(tile.position, MobTypeEnum.PURSUER) &&
        !hasMobs(tile) &&
...
export const replenishPursuers = (): void => {
  ...
  for (
    ...
    const mob = getGuaranteedRandomMob(spawnPosition, MobTypeEnum.PURSUER)
