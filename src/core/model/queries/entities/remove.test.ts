...
describe("entity removal", () => {
  ...
  describe("removeEntity", () => {
    describe("with entity object", () => {
      it("removes the entity recursively", () => {
        ...
        upsertRoleEntities(child, { [EntityRoleEnum.DEFAULT]: grandchild })
        upsertComponents(child, component)
        removeEntity(child)
        expectEntityRemoved(state, parent, child, EntityRoleEnum.ITEM)
  ...
  it("removes only children in selected roles", () => {
    ...
    upsertRoleEntities(parent, {
      ...
      [EntityRoleEnum.EQ]: eq,
    })

    removeEntitiesByRole(parent, EntityRoleEnum.ITEM)

    expectEntityRemoved(state, parent, item, EntityRoleEnum.ITEM)
    expectEntityAttached(state, parent, eq, EntityRoleEnum.EQ)
  })
  ...
  describe("detachEntity", () => {
    describe("with entity object", () => {
      it("detaches the entity from its parent", () => {
        ...
        upsertRoleEntities(parent, { [EntityRoleEnum.ITEM]: child })
        detachEntity(child)
        expectEntityDetached(state, parent, child, EntityRoleEnum.ITEM)
      })
    })
