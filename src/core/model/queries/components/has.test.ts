...
describe("component predicates", () => {
  ...
  describe("hasEqualComponentTypes", () => {
    it("returns true for no components", () => {
      expect(hasEqualComponentTypes()).toBe(true)
    })

    it("returns true with matching component creators", () => {
      expect(hasEqualComponentTypes(TestComponent, TestComponent)).toBe(true)
    })

    it("returns true with matching component objects", () => {
      const first = TestComponent()
      const second = TestComponent()
      expect(hasEqualComponentTypes(first, second)).toBe(true)
    })

    it("returns true with matching component types", () => {
      expect(
        hasEqualComponentTypes(TestComponent.type, TestComponent.type),
      ).toBe(true)
    })

    it("returns false for different component types", () => {
      expect(
        hasEqualComponentTypes(TestComponent(), AnotherTestComponent),
      ).toBe(false)
    })
  })
  ...
  describe("isComponentType", () => {
    ...
    it("returns true with matching component types", () => {
      expect(isComponentType(TestComponent.type, TestComponent.type)).toBe(true)
    })

    it("returns true with matching component type and creator", () => {
      expect(isComponentType(TestComponent, TestComponent.type)).toBe(true)
    })

    it("returns false for a different component type", () => {
      expect(isComponentType(TestComponent(), AnotherTestComponent.type)).toBe(
        false,
      )
    })
  })
  ...
  describe("hasComponentsByType", () => {
    describe("with entity object", () => {
      ...
      it("returns false with missing component type", () => {
        ...
        const component = TestComponent()
        upsertComponents(entity, component)
        expect(hasComponentsByType(entity, AnotherTestComponent.type)).toBe(
          false,
        )
      })
    })
